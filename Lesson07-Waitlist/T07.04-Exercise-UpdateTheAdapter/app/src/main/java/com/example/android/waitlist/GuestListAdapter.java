...
public class GuestListAdapter extends RecyclerView.Adapter<GuestListAdapter.GuestViewHolder> {
    ...
    public void onBindViewHolder(GuestViewHolder holder, int position) {
        ...
        final boolean moveResult = mCursor.moveToPosition(position);
        if (!moveResult) return;

        // DONE (6) Call getString on the cursor to get the guest's name
        final int columnIndexGuestName = mCursor.getColumnIndex(WaitlistEntry.COLUMN_GUEST_NAME);
        final String guestName = mCursor.getString(columnIndexGuestName);

        // DONE (7) Call getInt on the cursor to get the party size
        final int columnIndexPartySize = mCursor.getColumnIndex(WaitlistEntry.COLUMN_PARTY_SIZE);
        final int partySize = mCursor.getInt(columnIndexPartySize);
